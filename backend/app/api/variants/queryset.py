...
class SearchVarianQuerySet():
    ...
    def clean_params(self):
        print("SearchVarianQuerySet=>clean_params")

        # Clean params
        params = remove_key_thathas_emptystring_value(self.params)
        # print(f'params: {params}')
        try:
            self.params = SearchParamsSchema().load(params)
    ...
    def filter(self):
        ...
        if 'keyword' in self.params:
            ...
            if 'name' in self.search_list:
                conds_or.append(Variant.name.like(
                    '%{}%'.format(keyword)))
            if 'color' in self.search_list:
                conds_or.append(Variant.color.like(
                    '%{}%'.format(keyword)))
            if 'size' in self.search_list:
                conds_or.append(Variant.size.like(
                    '%{}%'.format(keyword)))


        # FILTER and_ & or_
        if len(conds_or) >= 1:
            self.objects = Variant.query.filter(
